...
const Feed = {
  ...
  createLike: async ({ feedId, userId }) => {
    try {
      ...
      const likeId = await promisePool.query(
        "SELECT id FROM feed_likes WHERE feed_id = ? AND user_id = ?",
        [feedId, userId]
      );
      return likeId;
    } catch (error) {
  ...
  findAllCommentsByFeedId: async ({ feedId }) => {
    try {
      const comments = await promisePool.query(
        "SELECT *, feed_comments.id as comment_id FROM feed_comments JOIN users ON feed_comments.user_id = users.id WHERE feed_comments.feed_id = ? ORDER BY feed_comments.id DESC",
        [feedId]
      );
